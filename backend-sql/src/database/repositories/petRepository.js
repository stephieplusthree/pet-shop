...
class PetRepository extends AbstractRepository {
  ...
  async create(data, options) {
    const record = await models.pet.create(
      {
  ...
  async update(id, data, options) {
    let record = await models.pet.findByPk(id, {
      transaction: AbstractRepository.getTransaction(
        options,
      ),
    });
  ...
  async _createOrUpdateFiles(record, data, options) {
    for (const field of this.fileAttributes) {
      await FileRepository.replaceRelationFiles(
        {
          belongsTo: models.pet.getTableName(),
  ...
  async findAndCountAll(
    ...
    let { rows, count } = await models.pet.findAndCountAll({
  ...
  async findAllAutocomplete(query, limit) {
    ...
    if (query) {
      ...
      filter.appendIlike('name', query, 'pet');
    }
